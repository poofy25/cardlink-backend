      ...
      validator: {
        validate(value: unknown) {
          ...
          if (isLinkWithMeta(value)) {
            ...
            if (value.meta !== null && typeof value.meta !== 'object')
              return false;

            // If we have both type and meta, check if it's a valid link type
            return Object.values(LinkTypes).includes(value.type);
          }
          ...
          if (Array.isArray(value)) {
            return value.every((link) => {
              ...
              if (link.meta !== null && typeof link.meta !== 'object')
                return false;

              return Object.values(LinkTypes).includes(link.type);
            });
          }
        ...
        defaultMessage(args: ValidationArguments) {
          ...
          if (isLinkWithMeta(value)) {
            return `Invalid link type '${value.type}'. Must be one of: ${Object.values(LinkTypes).join(', ')}`;
          }
